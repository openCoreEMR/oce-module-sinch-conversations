...
class AppListCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $apps = $apiClient->listApps();

            if (empty($apps)) {
                $io->warning('No apps found in this project.');
            ...
            foreach ($apps as $app) {
                $rows[] = [
                    $app['id'] ?? 'N/A',
                    $app['display_name'] ?? 'N/A',
                    count($app['channel_credentials'] ?? []),
                ];
            }

            $io->table(['App ID', 'Display Name', 'Channels'], $rows);
            $io->success(sprintf('Found %d app(s)', count($apps)));
