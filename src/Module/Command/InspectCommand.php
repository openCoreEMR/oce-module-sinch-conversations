        ...
        try {
            ...
            if (empty($appConfig['channel_credentials'])) {
                ...
                foreach ($appConfig['channel_credentials'] as $channel) {
                    ...
                    // Handle state - it can be a string or an array
                    $state = $channel['state'] ?? 'UNKNOWN';
                    if (is_array($state)) {
                        // State is an array with status
                    ...
                    $details = ["  State: {$stateStatus}"];

                    if (!empty($stateDescription)) {
                        $details[] = "  Description: {$stateDescription}";
                    }

                    // Service Plan ID (for SMS)
                    // Note: In older Sinch API versions, Service Plan ID is in claimed_identity
                    if (isset($channel['sms_service_plan_id'])) {
                        $details[] = "  Service Plan ID: {$channel['sms_service_plan_id']}";
                    } elseif ($channelType === 'SMS' && isset($channel['static_bearer']['claimed_identity'])) {
                        $claimedId = $channel['static_bearer']['claimed_identity'];
                        $details[] = "  Service Plan ID: {$claimedId} (via claimed_identity)";
                    } elseif ($channelType === 'SMS') {
                        $details[] = "  Service Plan ID: ⚠️  NOT CONFIGURED";
                    }
                    ...
                    if ($channelType === 'SMS') {
                        $servicePlanId = $channel['sms_service_plan_id']
                            ?? $channel['static_bearer']['claimed_identity']
                            ?? null;

                        if (empty($servicePlanId)) {
                            $io->error("  ❌ SMS Service Plan ID is REQUIRED to send messages!");
                            $io->note([
                                'To fix this:',
                                '  1. Create an SMS Service Plan at: https://dashboard.sinch.com',
                                '  2. Configure it with your app ID',
                                '  3. Update your channel configuration with the Service Plan ID',
                            ]);
                        } elseif (!preg_match('/^\+?\d+$/', (string) $servicePlanId)) {
                            // Alphanumeric sender detected
                            $io->warning("  ⚠️  Alphanumeric sender detected: {$servicePlanId}");
                            $io->note([
                                'Alphanumeric senders are NOT supported in many countries (including USA).',
                                'If messages are failing with CHANNEL_REJECT errors:',
                                '  1. Go to your Sinch Service Plan settings',
                                '  2. Configure a numeric originator (phone number or short code)',
                                '  3. Example: +14155551234 or a short code like 12345',
                            ]);
                        }
                    }
                }
